...
def test_two_bots_keep_separate_reply_keyboards_on_screen() -> None:
  ...
  _send_message(net, TOKEN, {"chat_id": "9", "text": "a", "reply_markup": _kb("Клуб")})
  _send_message(net, ALERT, {"chat_id": "9", "text": "b", "reply_markup": _kb("Алерт")})
  assert net.reply_keyboard(9, 111111111) == [["Клуб"]]
  assert net.reply_keyboard(9, 222222222) == [["Алерт"]]
  assert UserClient(BotView(net, TOKEN), 9).screen().reply_keyboard == [["Клуб"]]
  assert UserClient(BotView(net, ALERT), 9).screen().reply_keyboard == [["Алерт"]]
...
async def test_user_http_reads_keyboard_by_bot_peer() -> None:
  ...
  async with AsyncClient(transport=ASGITransport(app=app), base_url="http://tg") as client:
    ...
    await client.post(
      ...
      data={"chat_id": "9", "text": "a", "reply_markup": _kb("Клуб")},
    )
    await client.post(
      f"/bot{ALERT}/sendMessage",
      data={"chat_id": "9", "text": "b", "reply_markup": _kb("Алерт")},
    )
    await client.post("/user/sessions", json={"user_id": 9})
    club = (await client.get("/user/chats/111111111/messages")).json()
    alert = (await client.get("/user/chats/222222222/messages")).json()
    assert club["reply_keyboard"] == [["Клуб"]]
    assert alert["reply_keyboard"] == [["Алерт"]]
