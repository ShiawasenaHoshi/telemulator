...
async def test_reply_keyboard_survives_the_next_message() -> None:
  """Telegram держит клавиатуру, пока её не заменят или не уберут."""
  net = Network()
  net.create_user(id=9, first_name="Тест")
  net.create_bot(token=TOKEN)
  net.ensure_private_chat(9, 111111111)
  ...
  _send_message(net, TOKEN, {"chat_id": "9", "text": "Меню", "reply_markup": _menu("Расчёты КМ")})
  _send_message(net, TOKEN, {"chat_id": "9", "text": "И ещё сообщение"})
  assert user.screen().reply_keyboard == [["Расчёты КМ"]]
  assert user.screen().message.reply_keyboard is None

  _send_message(
    net,
    TOKEN,
    {"chat_id": "9", "text": "форма", "reply_markup": json.dumps({"remove_keyboard": True})},
  )
  assert user.screen().reply_keyboard is None


async def test_press_callback_finds_button_on_older_message() -> None:
  """Telegram не гасит инлайн-клавиатуры: кнопка живёт на старом сообщении."""
  net = Network()
  net.create_user(id=9, first_name="Тест")
  net.create_bot(token=TOKEN)
  net.ensure_private_chat(9, 111111111)
...
async def test_press_finds_the_button_when_the_human_talks_to_two_bots() -> None:
  """У человека может быть не один бот: ищем тред с сообщением, не первый."""
  net = Network()
  net.create_user(id=9, first_name="Тест")
  ...
  markup = json.dumps({"inline_keyboard": [[{"text": "Да", "callback_data": "yes"}]]})
  _send_message(net, ALERT_TOKEN, {"chat_id": "9", "text": "от второго", "reply_markup": markup})
