...
It stops working when the bot cannot be imported. This package requires Python 3.12. A bot pinned to an older interpreter runs in its own container and reaches the fake Bot API over the network, which the published image serves on `0.0.0.0:8081`.
...
0. **The Bot API answers 405 to a framework that uses GET.** `@router.post("/bot{token}/{method}")` reads its arguments with `await request.form()`. pyTelegramBotAPI 3.7.9 passes `method='post'` in 36 places and defaults to GET everywhere else, so a bot built on it cannot even poll. This gap gates every other one: until it is closed nothing downstream can be observed at all.
...
3. **`update_id` is discarded.** `send_text` and `press_callback` return it; `post_message` returns the stored message and `press` returns `query_id`. Neither passes the update on.

4. **Nothing to wait on.** `UserClient._wait` awaits `network.wait_acked(token, update_id, timeout)` — the offset acknowledgement, which arrives *after* the handler returns, when every message has been sent and every write committed. A remote caller cannot reach that.

Gap 4 is the subtle one. Waiting for a new message to appear is a weaker guarantee: it fires on the bot's first reply, not at the end of the handler, and a test that asserts on the database right after it is racing. Without the ack, a remote client degrades into waiting for silence — precisely what `wait_acked` was built to replace.

## 3. Design

### 3.1 Bot API

`/bot{token}/{method}` accepts GET as well as POST. Arguments come from the query string, and a POST body still wins where both are present. Nothing else about the handler changes: query values arrive as strings, exactly as form values already do.
...
`send`, `send_photo` and `send_document` gain a `reply_to_message_id` argument on **both** clients: gap 1 is fixed in `user_api`, below the client layer, so the in-process client would otherwise be the one left unable to reply.
...
- A bot built on pyTelegramBotAPI polls, answers and downloads a file against the emulator without a single 405.
